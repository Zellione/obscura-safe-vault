...
**Phase 102 (VAAPI diagnostics + force-software toggle):** the silent-no-op
runtime failure mode of `av_hwdevice_ctx_create` (wrong driver name, missing
`/dev/dri/renderD*` for the user, driver without the codec's VAProfile) is
surfaced as a one-time `[HwAccel]` log line on the first probe attempt —
