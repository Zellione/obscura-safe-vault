...
| FFmpeg/libav | 7.1.1 | Video & audio decode-only (H.264/H.265 + ProRes/DNxHD-DNxHR/MJPEG for `.mov` pro codecs, Phase 28; VP8/VP9 for `.webm`, Phase 38; aac/opus/mp3/vorbis/flac/ac3 audio; mov/mp4/m4v + matroska/webm demux; libswscale for video, swresample linked as transitive dependency of audio decoders — we do NOT use swresample for audio conversion, SDL_AudioStream handles that); configure-built static → `vendor/codecs-prefix`; needs **nasm**; linked by `link_av()` (avformat/avcodec/swscale/swresample/avutil) under `OSV_VENDORED_AV` (Phase 15–16) |
...
(`--disable-everything` then opt-in h264/hevc/prores/dnxhd/mjpeg/vp8/vp9 decoders (pro `.mov` codecs Phase 28; `.webm` VP8/VP9 Phase 38), aac/opus/mp3/vorbis/flac/ac3 audio decoders,
