...
int count_media(const Vault& v, std::string_view gallery)
{
    ...
    for (const auto* c : v.list(gallery)) {
        if (c->is_media())        ++n;
        else if (c->is_gallery())
            n += count_media(v, child_path(gallery, c->name.view()));
    }
    return n;
}
...
// The leaf case: every media file in src_gallery moves or copies into dst_gallery via
                                ...
                                std::string_view dst_gallery)
{
    ...
    for (const auto* c : ctx.src.list(src_gallery))
        if (c->is_media()) names.emplace_back(c->name.view());
                                       ...
                                       std::string_view dst_gallery)
{
    ...
    std::vector<std::string> names;
    for (const auto* c : ctx.src.list(src_gallery))
        if (c->is_gallery()) names.emplace_back(c->name.view());
                         ...
                         std::string_view dst_gallery)
{
    ...
    for (const auto& t : src_node->tags)
        (void)ctx.dst.add_tag(dst_gallery, t.view());
