...
std::vector<FreeRange> free_ranges(std::span<const Unit> units,
                                   std::span<const Unit> pinned,
                                   uint64_t floor)
{
    std::vector<std::pair<uint64_t, uint64_t>> live;  // (offset, length)
    live.reserve(units.size() + pinned.size());
    for (const Unit& u : units)  live.emplace_back(u.offset, u.length);
    for (const Unit& p : pinned) live.emplace_back(p.offset, p.length);
...
std::vector<Move> plan_pass(std::vector<Unit> units,
                            uint64_t floor,
                            std::span<const Unit> pinned)
{
    ...
    uint64_t max_free = 0;
    for (const FreeRange& r : free) max_free = std::max(max_free, r.length);

    for (auto it = units.rbegin(); it != units.rend(); ++it) {
        const Unit& u = *it;
        if (u.length > max_free) continue;
        for (FreeRange& r : free) {
            if (r.start >= u.offset) break;      // ranges are ascending; no dest below src left
...
{
    uint64_t end = floor;
    for (const Unit& u : units) end = std::max(end, u.offset + u.length);
    return end;
}
