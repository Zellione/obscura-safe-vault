...
## Post-PR: Known-Vault Picker on Unlock Screen (2026-07-30)

**Owner request:** the native file dialog cannot show hidden directories, and the per-user data folder (`~/.local/share/ObscuraSafeVault/ObscuraSafeVault/` via `SDL_GetPrefPath`) is where the default vault lives — so vaults there were unreachable from the Qt unlock screen.

**Implementation:**

1. **`vault_list_model.{h,cpp}` (new)** — `VaultListModel : QAbstractListModel` exposing known vaults to QML: entries from the SDL app's `vaults.list` registry (`platform::VaultRegistry::default_location().list()`, most-recent-first, each line already normalized via `platform::normalize_user_path` — invariant 6) plus a scan of `platform::config_dir()` for `*.osv` files (alphabetical, deduped against registry, existence-checked). Roles: `name` (filename), `dir` (parent path). `Q_INVOKABLE refresh()`, `Q_INVOKABLE fileUrlAt(row)`. **Read-only — never writes the registry.** Stores only file paths (same strings vaults.list holds); vault content and node names are never touched. Registered as context property `vaultListModel` in `AppContext` (both normal and selftest paths). `src/platform/vault_registry.cpp` added to `osv_qt_core`.
2. **`Main.qml` unlock screen** — ListView of known vaults above the file-dialog button (visible when count > 0; name + dimmed dir per row, selected row highlighted via `selectedVaultRow`); clicking a row calls `unlockController.openVault(vaultListModel.fileUrlAt(index))` and focuses the password field; button label becomes "Open Other Vault…" when the list is non-empty; `vaultListModel.refresh()` on screen completion so re-lock picks up changes.
