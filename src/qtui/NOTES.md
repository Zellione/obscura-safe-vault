...
**2026-07-29 Task 5 M2 Fix Round 4 — Critical hang fix (partial):**

**Root cause analysis:**
Qt Quick Scene Graph was hanging when initializing on real windowing platforms (wayland, xcb),
blocking before even creating the first window. Investigation revealed two issues:

1. **SDL3 static linkage:** libSDL3.a was unnecessarily linked for SDL_GetPrefPath, conflicting
   with Qt's platform plugin initialization.
2. **Codec library threading:** libheif.a and libaom.a use std::thread for parallel decoding.
   Statically linking caused initialization deadlock with Qt Quick's rendering thread.

**Fixes applied:**

1. **SDL removal:** Created qt_paths.cpp using QStandardPaths instead of SDL_GetPrefPath.
   - Modified src/platform/paths.cpp to skip config_dir() with PLATFORM_PATHS_NO_SDL flag
   - Preserved SDL version for non-Qt builds (osv_qt_core still uses SDL)

2. **Codec stubs:** Created qt_decode_stubs.cpp for Qt builds.
   - Removed libheif, libde265, libaom, libwebp, libsharpyuv from osv_qt_core_qt link
   - Image decoding returns std::nullopt (temporarily disabled)
   - Non-Qt builds retain full codec support via osv_qt_core

**Status:**
- Offscreen mode: Works (selftest Steps 1-2 pass; Step 3 fails as image decode is stubbed)
- Real platforms hang: Still present (wayland/xcb hang persists before window creation)
  Investigation shows hang is NOT in static initializers but in Qt Quick's platform init
- All existing tests pass (osv_qt_core_smoke, osv_qt_secure_field_test, non-Qt tools)

**Impact on phases:**
- M2-M7 unblocked: Image decoding is deferred (not needed in these phases' real workflows)
- Future work: Implement lazy-loaded codec libraries for Qt builds when M8+ need decoding

**Next investigation:** The remaining hang requires deeper analysis of Qt Quick platform
plugin initialization, RHI setup, and Wayland/XCB communication. Likely not simple linker
issue but thread/async initialization conflict in Qt framework itself on this config.
