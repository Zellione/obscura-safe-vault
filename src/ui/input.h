...
// True for the one key the image viewer routes to animated-image playback (Space,
// pause/resume). Every other key must fall through to the normal image handling
// so navigation still works while an animation is on screen (Phase 47).
[[nodiscard]] bool anim_viewer_consumes_key(SDL_Keycode key) noexcept;
