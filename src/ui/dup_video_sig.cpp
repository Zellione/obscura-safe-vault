...
// Stream state handed to the AVIO callbacks (mirrors media::ChunkAvio, but
// over the caller's thread-safe byte reader instead of a VideoSource).
struct Stream {
    const DupStreamRead& read;
    uint64_t             size;
    uint64_t             pos = 0;
};
...
int read_cb(void* opaque, uint8_t* buf, int buf_size)
{
    ...
    const int64_t n = s->read(s->pos, std::span<uint8_t>(buf, static_cast<size_t>(buf_size)));
    if (n < 0)  return AVERROR(EIO);
    if (n == 0) return AVERROR_EOF;
    s->pos += static_cast<uint64_t>(n);
...
{
    auto* s = static_cast<Stream*>(opaque);
    const auto size = static_cast<int64_t>(s->size);
    ...
    switch (whence) {
        ...
        case SEEK_CUR: new_pos = static_cast<int64_t>(s->pos) + offset; break;
        case SEEK_END: new_pos = size + offset; break;
        default:       return AVERROR(EINVAL);
    }
    if (new_pos < 0) return AVERROR(EINVAL);
    s->pos = static_cast<uint64_t>(new_pos);
    return new_pos;
}
...
bool decode_until(SigCtx& c, int64_t target_pts)
{
    ...
    while (decoded < SEEK_DECODE_CAP) {
        ...
        if (ret == 0) {
            ++decoded;
            const int64_t pts = c.frame->best_effort_timestamp;
            if (pts == AV_NOPTS_VALUE || pts >= target_pts) return true;
        ...
        // Feed the decoder the next packet of our stream; on demux EOF flush
        // ONCE and go back to draining (av_packet_unref resets stream_index,
        // so the post-EOF packet must never be sent).
        while (true) {
            av_packet_unref(c.pkt);
            if (av_read_frame(c.fmt, c.pkt) < 0) {
                if (avcodec_send_packet(c.codec, nullptr) < 0) return false;
                flushed = true;
                break;
            }
            if (c.pkt->stream_index == c.stream_index) {
                if (avcodec_send_packet(c.codec, c.pkt) < 0) return false;
                break;
            }
        }
    }
...
bool frame_to_hash(SigCtx& c, uint64_t& out_hash)
{
    ...
    const int dst_stride[1] = {HASH_SIDE * 3};
    if (sws_scale(c.sws, c.frame->data, c.frame->linesize, 0, c.frame->height,
                  dst, dst_stride) <= 0)
        return false;
...
{
    Stream stream{.read = read, .size = total_size};
    SigCtx c;
    ...
    for (size_t i = 0; i < DUP_VID_FRAME_POSITIONS.size(); ++i) {
        ...
        uint64_t hash = 0;
        if (decode_until(c, target_pts) && frame_to_hash(c, hash)) {
            sig.frame_hash[i] = hash;
            sig.frame_valid |= static_cast<uint8_t>(1u << i);
        }
        av_frame_unref(c.frame);
    }
    return true;
