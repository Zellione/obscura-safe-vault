...
// and the session's "keep unlocked" toggle all suppress the timer and reset it,
// so a later re-enable starts counting from zero instead of a stale elapsed
// value. Pure / SDL-free so the branches are unit-testable without an App.
[[nodiscard]] inline bool should_auto_lock(bool has_active, bool blocks_idle_lock,
                                           bool keep_unlocked, IdleTimer& timer,
                                           double dt) noexcept
{
    if (!has_active || blocks_idle_lock || keep_unlocked) {
        timer.reset();
        return false;
    }
