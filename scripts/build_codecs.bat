...
rem libheif ties it together; decoders are baked in statically (no plugin loading)
rem and it finds libde265/libaom via CMAKE_PREFIX_PATH. libheif >= 1.19 needs C++20
rem and defaults WITH_X264 and WITH_OpenH264_DECODER ON; both MUST be disabled to
rem avoid pulling in an AVC encoder/decoder (Phase 95 / OSV-AUD-002).
set "EXTRA=-DWITH_LIBDE265=ON -DWITH_AOM_DECODER=ON -DWITH_AOM_ENCODER=OFF -DWITH_X265=OFF -DWITH_X264=OFF -DWITH_OpenH264_DECODER=OFF -DWITH_EXAMPLES=OFF -DWITH_GDK_PIXBUF=OFF -DENABLE_PLUGIN_LOADING=OFF -DBUILD_TESTING=OFF -DCMAKE_DISABLE_FIND_PACKAGE_Doxygen=ON -DCMAKE_C_FLAGS=-DLIBDE265_STATIC_BUILD -DCMAKE_CXX_FLAGS=-DLIBDE265_STATIC_BUILD"
