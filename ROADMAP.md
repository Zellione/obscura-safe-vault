...
### Tasks
- [ ] **Add FFmpeg decoders** — extend `--enable-decoder` in
  ...
  matroska demuxer packetizes VP8/VP9 frames itself). Decode-only, as with
  every other codec in this build. `build_codecs.bat` stays untouched
  (video remains `OSV_VENDORED_AV`-gated to Linux/macOS, per the existing
  Windows FFmpeg carve-out).
- [ ] **`VideoCodec` enum** (`src/vault/index.h`) — add `VP8`/`VP9` values
...
- [ ] **Confirm audio path unchanged** — Opus/Vorbis decode already works
...
- [ ] Update `CLAUDE.md` (FFmpeg decoder list, `VideoCodec` table) + the
  README stack line + `docs/VENDORED_DEPS.md` (if the decoder list is
  tracked there) + `mem:tech_stack`.
- [ ] `tests/` — small `.webm` fixtures (VP8 and VP9, each with and without
