...
| 62 | Perceptual video duplicates | 🔜 | Extend Phase 61's "Exact + visually similar" scan to catch re-encoded / resized / remuxed copies of the same video (today only byte-identical videos group). Three-stage funnel, cheapest evidence first: duration gate from `vmeta.duration_us` (±2 %, 500 ms floor — zero I/O), poster dHash prefilter (stored first-frame JPEG, Hamming ≤ 10), then a frame confirm that software-decodes 5 frames at 10–90 % of the timeline through a thread-safe `read_thumb_span`-backed AVIO (the worker must never touch the main-thread `read_fp_`/`VideoSource`) and requires ≥ 4 of 5 per-position dHashes within Hamming ≤ 7. Union-find clusters pairs into "Similar video" groups; review/apply unchanged. Non-FFmpeg builds accept the duration+poster verdict. No new UI mode, no `.osv` change. Design approved by owner 2026-08-03. → [details](docs/roadmap/phase-62-perceptual-video-duplicates.md) |
