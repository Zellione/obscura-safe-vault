...
### Tasks
- [ ] `src/crypto/random.{h,cpp}` — platform CSPRNG shim: `getrandom` (Linux), `getentropy` (macOS), `BCryptGenRandom` (Windows). Exposed as `crypto::fill_random(std::span<uint8_t>)`.
- [ ] `src/crypto/secure_mem.{h,cpp}` — `SecureBuffer<N>`: stack/heap buffer with `mlock` on construction and `crypto_wipe` + `munlock` on destruction.
- [ ] `src/crypto/kdf.{h,cpp}` — `crypto::derive_key(password, keyfile_opt, salt, params) -> SecureBuffer<32>` wrapping `crypto_argon2i` (Argon2id variant).
- [ ] `src/crypto/aead.{h,cpp}` — `encrypt_chunk` / `decrypt_chunk` wrapping `crypto_aead_lock` / `crypto_aead_unlock` (XChaCha20-Poly1305). Generates fresh random nonce per encrypt call.
- [ ] `tests/crypto/` — unit tests:
  - [ ] Known-answer tests for Argon2id (vectors from Monocypher test suite / RFC 9106).
  - [ ] Known-answer tests for XChaCha20-Poly1305 (vectors from RFC 8439 / Monocypher suite).
  - [ ] Round-trip: encrypt → decrypt → compare plaintext.
  - [ ] Tamper detection: flip one ciphertext byte → `decrypt_chunk` must fail.
  - [ ] `SecureBuffer` wipe verification (read after destruction returns zeroes with ASAN/manual check).
