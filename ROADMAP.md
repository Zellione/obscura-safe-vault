...
| 68 | Import-error resilience, multiselect everywhere & browsing polish | 🔜 | Corrupt-archive import no longer kills the app (task-boundary catch on the import worker → Failed task + `error.log` entry, root-cause hardened); multi-select drives favorite toggle, export, and move/copy on the grid (mixed images+videos+galleries in one batch) and gains full Space/`Ctrl+A` support with `B`/`X`/`M` on the favorites/tag/search screens; mouse wheel scrolls the viewer thumbnail strip and the saved-search sidebar under the cursor; `n / N` position counter on the grid footer and as a viewer-strip overlay (visible in fullscreen). → [details](docs/roadmap/phase-68-import-resilience-multiselect-polish.md) |
