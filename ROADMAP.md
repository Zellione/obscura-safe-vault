...
| 75 | Pull transfers, tag persistence, 512px thumbnails | ✅ | Three independent features. **(a) Pull transfers:** TransferDialog gains a Direction stage (M: Push=copy, Pull=copy from source, Symmetric=swap metadata and unlink tags from source on cut) — `Push` is today's copy semantics, `Pull` copies files + metadata from a multi-select source-gallery picker and materializes inherited tags (every media leaf's effective tags become its own; every subtree root gains `_ancestors_above` virtual tag holding ancestors' tags). Symmetric exchange copies to destination and re-wraps source files with destination metadata (tags/dates), then applies to source's own gallery only — no subtree untagging (tags stay where used). **(b) Tag & favorite persistence:** cross-vault transfer carries tags on every file (copy and Move), and applied tags + inherited ancestors materialize on arrival — if a favorite, keep favorited; if tagged with ancestors, those tags copy too (effective_tags → own tags, _ancestors_above → materialized ancestors on the import gallery). **(c) 512px thumbnails:** stored thumbnail size increased from 256 px to 512 px (`THUMB_MAX_SIDE`), tile sizes updated (S 192→192, M 256→256, L 248→352, XL 320→448), migration at unlock regenerates old thumbs to new size with one-time `migrated_thumb_side` watermark in settings (Index v12). TextureCache budget 256 MiB still holds; a 512 px thumb is ~1 MiB in the worst case. Backward-compatible: pre-v12 vaults lock in old tile sizes via the `migrated_thumb_side = 0` mark. 1999 tests / 0 failed. → [details](docs/roadmap/phase-75-pull-transfers-tag-persistence-bigger-thumbs.md) |
...
>   thumbnail migration watermark appended to the settings block 
>   (`migrated_thumb_side u16` — the target pixel size after migration, 0 for 
>   pre-v12 vaults that never migrate), bumping `INDEX_VERSION` to **12**; pre-v12 
>   blobs read `0` and lock in 256px thumbnails. Index v12 vaults created fresh 
>   stamp `migrated_thumb_side = 512`. Existing vaults landing on v12 via migration 
>   trigger a one-time recompute of all thumbnails at the new size at unlock 
>   (async `MigrationJob` before browsing begins); the watermark updates to 512 
>   after success. An out-of-range watermark value is rejected on deserialise, 
>   not clamped.
