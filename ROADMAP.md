...
| 40 | Video codec/loop/sync polish, gallery position memory & view density | 🔜 | Part 1 ✅: AV1 + broader `.mov` codecs, video loop toggle, A/V sync hardening, + bugfix ✅: self-healing metadata repair for videos imported before their codec was decodable. Parts 2/3 not started: session-scoped sub-gallery/selection memory, 5-way List/Grid S-XL view density. → [details](docs/roadmap/phase-40-video-gallery-browsing-polish.md) |
