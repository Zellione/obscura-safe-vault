...
| 50 | Background import queue | ⏳ | All imports (picked files, zip/cbz, 7z/rar/tar) run on a queueable background pipeline while the vault stays fully browsable — main-thread-only index tree, second read-only file handle, thread-safe chunk staging, coalescing async batched index commits (~32 files / 2 s), parallel thumbnail-decode pool. New Import Status screen (`Shift+I`: progress, reorder, cancel) + live footer summary; idle auto-lock suppressed while the queue runs, confirm dialog on manual lock/quit. → [details](docs/roadmap/phase-50-background-import-queue.md) |
