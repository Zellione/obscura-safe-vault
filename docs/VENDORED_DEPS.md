...
| **FFmpeg** | 7.1.1 (n7.1.1) | Video and audio frame decoding (decode-only, static linked; H.264/H.265 + ProRes/DNxHD/MJPEG since Phase 28; VP8/VP9 since Phase 38; AV1 via libaom + QTRLE/Cinepak since Phase 40; GIF demuxer + decoder since Phase 47; legacy containers AVI/MPEG-PS/MPEG-TS/ASF/FLV/Ogg/RealMedia + ~34 legacy codecs since Phase 52; libavfilter linked for yadif deinterlacing since Phase 52; VAAPI hwaccel enabled (Linux-only since Phase 101; Phase 102 surfaces the runtime probe outcome via the `[HwAccel]` log + `media::hwaccel_status()` so a failure to attach the GPU decode block is no longer silent) | **Yes** |
