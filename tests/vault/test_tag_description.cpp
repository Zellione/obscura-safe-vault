...
TEST(tag_description_round_trips_through_serialisation)
{
    VaultSettings s;
    s.tag_descriptions = {
        {.tag = crypto::SecureString("beach"), .text = crypto::SecureString("Coastal shots only")},
        {.tag = crypto::SecureString("artist:Kaguya"),
         .text = crypto::SecureString("Active 2011-2019")}};
...
TEST(tag_description_over_length_text_is_rejected_not_clamped)
{
    ...
    VaultSettings s;
    s.tag_descriptions = {
        {.tag = crypto::SecureString("beach"), .text = crypto::SecureString("ok")}};
    std::vector<uint8_t> blob;
    serialize_index(make_root(), {}, s, blob);
