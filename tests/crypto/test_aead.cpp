...
{
    static const char plaintext_str[] =
        "Ladies and Gentlemen of the class of '99: If I could offer you "
        "only one tip for the future, sunscreen would be it.";
    const size_t plen = sizeof(plaintext_str) - 1;  // 114 bytes, drop NUL
    REQUIRE(plen == 114);
    ...
    static constexpr uint8_t expected_cipher[114] = {
    ...
    static constexpr uint8_t expected_tag[16] = {
    ...
    crypto_aead_lock(cipher.data(), tag.data(), key.data(), nonce.data(),
                     ad.data(), ad.size(),
                     reinterpret_cast<const uint8_t*>(plaintext_str), plen);

    CHECK_BYTES_EQ(std::span<const uint8_t>(cipher),
                   std::span<const uint8_t>(expected_cipher, plen));
    CHECK_BYTES_EQ(std::span<const uint8_t>(tag),
                   std::span<const uint8_t>(expected_tag, 16));
}
...
TEST(tamper_detection)
{
    ...
    const size_t positions[] = {0 /*nonce*/, cipher_off /*ciphertext*/, tag_off /*tag*/};
