...
TEST(auto_lock_false_and_resets_when_no_active_vault)
{
    ...
    CHECK_FALSE(app::should_auto_lock(/*has_active=*/false, /*blocks_idle_lock=*/false,
                                      /*keep_unlocked=*/false, /*import_busy=*/false, t, 1.0));
...
TEST(auto_lock_false_and_resets_when_keep_unlocked)
{
    ...
    CHECK_FALSE(app::should_auto_lock(/*has_active=*/true, /*blocks_idle_lock=*/false,
                                      /*keep_unlocked=*/true, /*import_busy=*/false, t, 100.0));  // far past timeout
    CHECK_EQ(t.elapsed(), 0.0);
}
...
{
    app::IdleTimer t(5.0);
    CHECK_FALSE(app::should_auto_lock(true, false, false, false, t, 3.0));
    CHECK(app::should_auto_lock(true, false, false, false, t, 3.0));   // 6.0 >= 5.0 -> fire
...
TEST(auto_lock_disabling_keep_unlocked_starts_counting_fresh)
{
    ...
    CHECK_FALSE(app::should_auto_lock(true, false, true, false, t, 1000.0));
    CHECK_FALSE(app::should_auto_lock(true, false, false, false, t, 1.0));
...
TEST(auto_lock_false_and_resets_when_import_busy)
{
    ...
    (void)t.tick(3.0);
    CHECK_FALSE(app::should_auto_lock(/*has_active=*/true, /*blocks_idle_lock=*/false,
                                      /*keep_unlocked=*/false, /*import_busy=*/true, t, 1.0));
    CHECK_EQ(t.elapsed(), 0.0);
}
...
TEST(auto_lock_disabling_import_busy_starts_counting_fresh)
{
    ...
    CHECK_FALSE(app::should_auto_lock(true, false, false, true, t, 1000.0));
    CHECK_FALSE(app::should_auto_lock(true, false, false, false, t, 1.0));
    CHECK_EQ(t.elapsed(), 1.0);
}
