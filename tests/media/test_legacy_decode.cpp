...
// NOTE (Phase 52): MPEG-1/2 in MPEG-PS (.mpg) container cannot be reliably decoded
// through the vault's chunked AVIO interface. MPEG-PS is a program stream format that
// requires sequential access and doesn't support random seeking well, which conflicts
// with the vault's chunk-based storage model. These codecs are registered (Task 1) but
// are moved to Task 9 registration-only due to this incompatibility.
...
// MPEG-1 + MP2 audio test excluded - see comment above for MPEG-PS incompatibility.
...
// Interlaced MPEG2 test excluded - see comment above for MPEG-PS incompatibility.
// This will be covered in Task 9 with a different container format if needed.
